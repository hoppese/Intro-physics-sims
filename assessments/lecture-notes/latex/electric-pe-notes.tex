...
% even the sheet/plate figures) -- students draw the ball-and-ground and
% the charge-in-a-field themselves -- except a pair of bare axes for
% sketching U(theta). Major standalone equations are blanked as a labeled
...
% See electric-pe-notes-key.tex for the filled-in, all-diagrams version.
%
...
% (-0.5mm) suits a short answer (mgd, qEd, -pE\cos\theta). The two
% point-charge formulas are two-level fractions (one of them a fraction
% AND a sum-with-limits) -- measured directly off electric-pe-notes-key.pdf
% at 300dpi, U(q_1,q_2)'s typeset ink is ~0.43in tall and U_total's is
% ~0.54in, so their blanks pass an explicit taller gap.
...
\begin{itemize}[itemsep=9pt,parsep=4pt,topsep=4pt]
\item This is exactly like raising the ball: you do \bl[2.4cm]\ work
